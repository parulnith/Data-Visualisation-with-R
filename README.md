...
Now, load the file with the help of **read** command. In this case data is inform of a csv file named **airquality.csv** and can be downloaded from [here](https://github.com/parulnith/Data-Visualisation-with-R/blob/master/airquality.csv)
...
* **str(data)**
It displays the internal structure of an R object and gives a quick overview about regarding the rows and columns of the dataset.
...
We get a histogram of the **Solar.R** values. However, we can convert this plain histogram to a more visually appealing one by tweaking the arguments likr color etc. 
...
We have seen how the `summary()` command in R can  display the  descriptive statistics for every variable in the dataset. Boxplot does the same albeit graphically in form of quartiles. it is again very easy to plot a boxplot in R.
...
There is a very interesting feature in R which enables us to plot multiple charts at once. this comes very handy during EDA since the need to plot multiple graphs one by one gets eliminated. The first argument should specify certain attributes like  the margin of the grid(`mar)`, no of rows and columns(`mfrow`), whether border is to be included(`bty`) and position of the labels(`las`: 1 for horizontal, 0 for vertical). 
